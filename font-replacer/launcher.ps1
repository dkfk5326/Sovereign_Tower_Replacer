...
$script = Join-Path $base 'sovereign_font_replacer.py'
$log = Join-Path $base 'launcher_error.txt'
function Fail([string]$m) {
  try { Set-Content -LiteralPath $log -Value $m -Encoding UTF8 } catch {}
  try {
    Add-Type -AssemblyName System.Windows.Forms
    [System.Windows.Forms.MessageBox]::Show($m,'Sovereign Tower Font Replacer') | Out-Null
  } catch {}
  exit 1
}
if (-not (Test-Path -LiteralPath $script)) { Fail 'sovereign_font_replacer.py not found.' }
